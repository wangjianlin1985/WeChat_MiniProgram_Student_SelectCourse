...
public class CourseInfoController extends BaseController {
	...
	public void list(String courseNumber,String courseName,@ModelAttribute("teacherObj") Teacher teacherObj,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (courseNumber == null) courseNumber = "";
		if (courseName == null) courseName = "";
		if(rows != 0)courseInfoService.setRows(rows);
		List<CourseInfo> courseInfoList = courseInfoService.queryCourseInfo(courseNumber, courseName, teacherObj, page);
	    /*计算总的页数和总的记录数*/
	    courseInfoService.queryTotalPageAndRecordNumber(courseNumber, courseName, teacherObj);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String courseNumber,String courseName,@ModelAttribute("teacherObj") Teacher teacherObj,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (courseNumber == null) courseNumber = "";
		if (courseName == null) courseName = "";
		List<CourseInfo> courseInfoList = courseInfoService.queryCourseInfo(courseNumber, courseName, teacherObj, currentPage);
	    /*计算总的页数和总的记录数*/
	    courseInfoService.queryTotalPageAndRecordNumber(courseNumber, courseName, teacherObj);
	    /*获取到总的页码数目*/
	    int totalPage = courseInfoService.getTotalPage();
	...
	public void OutToExcel(String courseNumber,String courseName,@ModelAttribute("teacherObj") Teacher teacherObj, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(courseNumber == null) courseNumber = "";
        if(courseName == null) courseName = "";
        List<CourseInfo> courseInfoList = courseInfoService.queryCourseInfo(courseNumber,courseName,teacherObj);
        ...
        for(int i=0;i<courseInfoList.size();i++) {
        	...
        	dataset.add(new String[]{courseInfo.getCourseNumber(),courseInfo.getCourseName(),courseInfo.getTeacherObj().getTeacherName(),courseInfo.getCourseTime(),courseInfo.getCoursePlace(),courseInfo.getCourseScore() + ""});
        }
        /*

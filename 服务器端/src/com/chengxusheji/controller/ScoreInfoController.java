...
public class ScoreInfoController extends BaseController {
	...
	public void add(@Validated ScoreInfo scoreInfo, BindingResult br,
		...
		if(courseSelectService.queryCourseSelect("", scoreInfo.getStudentObj(), scoreInfo.getCourseObj()).size() == 0) {
			message = "此学生没有选择此门课程！";
			writeJsonResponse(response, success, message);
			return ;
		}
		
		if(scoreInfoService.queryScoreInfo(scoreInfo.getStudentObj(), scoreInfo.getCourseObj()).size() > 0) {
			message = "已经添加过此学生此门课程成绩了！";
			writeJsonResponse(response, success, message);
			return ;
		}
	...
	public void list(@ModelAttribute("studentObj") Student studentObj,@ModelAttribute("courseObj") CourseInfo courseObj,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if(rows != 0)scoreInfoService.setRows(rows);
		List<ScoreInfo> scoreInfoList = scoreInfoService.queryScoreInfo(studentObj, courseObj, page);
	    /*计算总的页数和总的记录数*/
	    scoreInfoService.queryTotalPageAndRecordNumber(studentObj, courseObj);
	    /*获取到总的页码数目*/
	...
	public String frontlist(@ModelAttribute("studentObj") Student studentObj,@ModelAttribute("courseObj") CourseInfo courseObj,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		List<ScoreInfo> scoreInfoList = scoreInfoService.queryScoreInfo(studentObj, courseObj, currentPage);
	    /*计算总的页数和总的记录数*/
	    scoreInfoService.queryTotalPageAndRecordNumber(studentObj, courseObj);
	    /*获取到总的页码数目*/
	    int totalPage = scoreInfoService.getTotalPage();
	...
	public void OutToExcel(@ModelAttribute("studentObj") Student studentObj,@ModelAttribute("courseObj") CourseInfo courseObj, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        List<ScoreInfo> scoreInfoList = scoreInfoService.queryScoreInfo(studentObj,courseObj);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "ScoreInfo信息记录"; 
        String[] headers = { "记录编号","学生","课程","成绩得分"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<scoreInfoList.size();i++) {
        	ScoreInfo scoreInfo = scoreInfoList.get(i); 
        	dataset.add(new String[]{scoreInfo.getScoreId() + "",scoreInfo.getStudentObj().getStudentName(),scoreInfo.getCourseObj().getCourseName(),scoreInfo.getScoreValue() + ""});
        }
        /*

...
public class CourseInfoService {
    ...
    /*按照查询条件分页查询课程信息记录*/
    public ArrayList<CourseInfo> queryCourseInfo(String courseNumber,String courseName,Teacher teacherObj,int currentPage) throws Exception { 
    	...
    	if(null != teacherObj &&  teacherObj.getTeacherNumber() != null  && !teacherObj.getTeacherNumber().equals(""))  where += " and t_courseInfo.teacherObj='" + teacherObj.getTeacherNumber() + "'";
    	int startIndex = (currentPage-1) * this.rows;
    	return courseInfoMapper.queryCourseInfo(where, startIndex, this.rows);
    }

    /*按照查询条件查询所有记录*/
    public ArrayList<CourseInfo> queryCourseInfo(String courseNumber,String courseName,Teacher teacherObj) throws Exception  { 
     	String where = "where 1=1";
    	if(!courseNumber.equals("")) where = where + " and t_courseInfo.courseNumber like '%" + courseNumber + "%'";
    	if(!courseName.equals("")) where = where + " and t_courseInfo.courseName like '%" + courseName + "%'";
    	if(null != teacherObj &&  teacherObj.getTeacherNumber() != null && !teacherObj.getTeacherNumber().equals(""))  where += " and t_courseInfo.teacherObj='" + teacherObj.getTeacherNumber() + "'";
    ...
    /*当前查询条件下计算总的页数和记录数*/
    public void queryTotalPageAndRecordNumber(String courseNumber,String courseName,Teacher teacherObj) throws Exception {
     	String where = "where 1=1";
    	if(!courseNumber.equals("")) where = where + " and t_courseInfo.courseNumber like '%" + courseNumber + "%'";
    	if(!courseName.equals("")) where = where + " and t_courseInfo.courseName like '%" + courseName + "%'";
    	if(null != teacherObj &&  teacherObj.getTeacherNumber() != null && !teacherObj.getTeacherNumber().equals(""))  where += " and t_courseInfo.teacherObj='" + teacherObj.getTeacherNumber() + "'";
